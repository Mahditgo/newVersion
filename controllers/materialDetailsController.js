...
exports.uplaodMaterial = async (req, res) => {
    try {
    ...
    const items = data.map((row) => ({
      ...
      endAmount: parseFloat(row[11]) || 0,
        ...
        const existingRecord = await materialModel.findOne({ reportId: req.body.reportId });
...
exports.getMaterialsDetails = async (req, res) => {
    try {
        
        const { reportId } = req.params;

        
        const records = await materialModel.find({reportId}); 
...
exports.deleteMaterialDetails = async (req, res) => {
    try {
        const { id } = req.params;
        const materials = await materialModel.findById(id);

        if (!materials) {
            return res.status(404).json({
                success: false,
                message: 'No materils found with the provided reportId',
            });
        }

        
        if (materials.filePaths && materials.filePaths.length > 0) {
            for (const filePath of materials.filePaths) {
                try {
            ...
            console.warn('No filePaths available for this materils.');
        }
        ...
        if (!res.headersSent) {
            return res.status(200).json({
                success: true,
                message: 'materialsdetail and its files were successfully deleted',
            });
        }
    } catch (error) {
        console.error(`Error in deleting materials: ${error.message}`);
